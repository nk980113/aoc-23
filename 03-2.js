const input = `.............65..................998.........453...................................845..773.........................307....527...........541
    ...
    input.split('\n').forEach((line, y) => {
        ...
        line.split('').forEach((char, x) => {
            if (!isNaN(char)) {
                ...
                if (digits.length > 0) {
                    numbers.push({
                        data: Number(digits.join('')),
                        y,
                        startX: x - digits.length,
                        len: digits.length,
                    });
                    digits = [];
                }
        ...
        if (digits.length > 0) {
            numbers.push({
                data: Number(digits.join('')),
                y,
                startX: line.length - digits.length,
                len: digits.length,
            });
        }
    });
    outer: for (const { data, y, startX, len } of numbers) {
        const endX = startX + len - 1;
        const middleRow = gears[y];
        if (middleRow) {
            if (middleRow[startX - 1]) {
                middleRow[startX - 1].push(data);
            }
            if (middleRow[endX + 1]) {
                middleRow[endX + 1].push(data);
            }
        }
        const upperRow = gears[y - 1];
        if (upperRow) {
            for (let i = startX - 1; i <= endX + 1; i++) {
                if (upperRow[i]) {
                    upperRow[i].push(data);
                }
            }
        }
        const lowerRow = gears[y + 1];
        if (lowerRow) {
            for (let i = startX - 1; i <= endX + 1; i++) {
                if (lowerRow[i]) {
                    lowerRow[i].push(data);
                }
            }
        }
    }
    let sum = 0;
    gears.forEach((row) => row.forEach((gear) => {
