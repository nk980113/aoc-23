const input = `F---7-FJ.7--FJ7.-.7-|7F.|7FJ77F7J-|--.FLF|.F-7-F7.F.J77|--|7--|7|-J-F--7-F7.F7-F7F7F-|-|-F777L7|-.FLF|-77F7-FJF|JF--F.77J-L7-FF--7.FFF7FF7.7
...
const pipeFuncMap = {
    '|'(x, y) {
        return [coordinate(x, y - 1), coordinate(x, y + 1)];
    },
    '-'(x, y) {
        return [coordinate(x - 1, y), coordinate(x + 1, y)];
    },
    'L'(x, y) {
        return [coordinate(x, y - 1), coordinate(x + 1, y)];
    },
    'J'(x, y) {
        return [coordinate(x, y - 1), coordinate(x - 1, y)];
    },
    ...
    'S'(x, y) {
        return [coordinate(x, y - 1), coordinate(x, y + 1), coordinate(x - 1, y), coordinate(x + 1, y)];
    },
};
...
function main() {
    ...
    const pipes = input.split('\n').map((l, y) => l.split('').map((c, x) => {
        if (c === 'S') start = coordinate(x, y);
        return [coordinate(x, y), pipeFuncMap[c](x, y)];
    })).flat();
    const digraph = new Map(pipes);
    const graph = new Map(pipes.map(([coord, connected]) => {
        const doubleConnected = connected.filter((target) => (digraph.get(target) ?? []).includes(coord));
        if (doubleConnected.length > 0) return [coord, doubleConnected];
    }).filter(Boolean));
    const searched = new Map();
    searched.set(start, 0);
    const searchQueue = [start];
    while (searchQueue.length > 0) {
        const curr = searchQueue.shift();
        const neighborDist = searched.get(curr) + 1;
        const neighbors = graph.get(curr);
        neighbors.forEach((coord) => {
            if (!searched.has(coord)) {
                searchQueue.push(coord);
                searched.set(coord, neighborDist);
            }
        });
    }
    return Math.max(...searched.values());
}
