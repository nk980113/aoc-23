...
function main() {
    ...
    lines.forEach((v, y) => v.split('').forEach((c, x) => map.set(coordinate(x, y), c)));
    const edges = [];
    const width = lines[0].length;
    const height = lines.length;
    for (let i = 0; i < width; i++) {
        edges.push([coordinate(i, -1), 2]);
        edges.push([coordinate(i, height), 0]);
    }
    for (let i = 0; i < height; i++) {
        edges.push([coordinate(-1, i), 1]);
        edges.push([coordinate(width, i), 3]);
    }
    
    const counts = edges.map((e) => getCount(map, ...e));

    return Math.max(...counts);
}

function getCount(map, start, dir) {
    const energized = new Set();
    const edgeSet = new Set();
    
    search(map, energized, edgeSet, start, dir);
