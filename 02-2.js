const input = `Game 1: 4 green, 7 blue; 2 blue, 4 red; 5 blue, 2 green, 2 red; 1 green, 3 red, 9 blue; 3 green, 9 blue; 7 green, 2 blue, 2 red
...
function main() {
    ...
    return input.split('\n').map((line) => {
        ...
        for (const round of rounds.split('; ')) {
            for (const show of round.split(', ')) {
                const [strCount, color] = show.split(' ');
                const count = Number(strCount);
                req[colorToIdx[color]] = Math.max(req[colorToIdx[color]], count);
            }
        }
        return req.reduce((p, c) => p * c, 1);
    }).reduce((p, c) => p + c, 0);
}
