...
function main() {
    ...
    const rows = input.split('\n').map((s) => {
        ...
        let firstPotentials = [];
        const toRemove = [];
        first_loop: for (const c of s) {
            if (!isNaN(Number(c))) {
                first = Number(c);
                break;
            }
            firstPotentials.push('');
            firstPotentials = firstPotentials.map((s) => s + c);
            for (const [i, v] of firstPotentials.entries()) {
                ...
                if (readyToRemove) toRemove.push(i);
            }
            firstPotentials = firstPotentials.filter((_, i) => {
                if (i === toRemove[0]) {
                    toRemove.shift();
                    return false;
                }
        ...
        let last;
        let lastPotentials = [];
        last_loop: for (const c of s.split('').reverse()) {
            if (!isNaN(Number(c))) {
                last = Number(c);
                break;
            }
            lastPotentials.push('');
            lastPotentials = lastPotentials.map((s) => s + c);
            for (const [i, v] of lastPotentials.entries()) {
                let readyToRemove = true;
                for (const [ni, n] of reversedNumbers.entries()) {
                    if (n.lastIndexOf(v) === 0) {
                        if (n.length === v.length) {
                            last = ni + 1;
                            break last_loop;
                        }
            ...
            lastPotentials = lastPotentials.filter((_, i) => {
                if (i === toRemove[0]) {
                    toRemove.shift();
                    return false;
                }
